...
def _inference_single_pose_model(model,
    ...
    with torch.no_grad():
        ...
        # result.keys():preds boxes image_paths
        # bbox_ids output_heatmap

    return result['preds'], result['output_heatmap']


def RM_inference_single_pose_model(model,
                                 img_or_path,
                                 bboxes,
                                 dataset='TopDownCocoDataset',
                                 dataset_info=None,
    ...
    return result['preds'], result['gaze'], result['output_heatmap']
...
def RMinference_top_down_pose_model(model,
                                  img_or_path,
                                  person_results=None,
                                  bbox_thr=None,
                                  format='xywh',
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        # poses is results['pred'] # N x 17x 3
        poses, gazes, heatmap = RM_inference_single_pose_model(
            model,
            img_or_path,
            bboxes_xywh,
            dataset=dataset,
        ...
        # print('type(gaze)',type(gazes))
        # print('type(heatmap)',type(heatmap))
        # print(poses.shape)
        # print(gazes.shape)
        # print(heatmap.shape)
    ...
    assert len(poses) == len(person_results), print(
        len(poses), len(person_results), len(bboxes_xyxy))
    for pose, gaze, person_result, bbox_xyxy in zip(poses, gazes, person_results,
                                              bboxes_xyxy):
        pose_result = person_result.copy()
        pose_result['keypoints'] = pose
        pose_result['bbox'] = bbox_xyxy
        pose_result['gaze'] = gaze
        pose_results.append(pose_result)
